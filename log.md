...
- 예전에 삭제됐던 "학번 미등록 구성원은 임시 비밀번호 CAP_이름을 입력하세요." 안내 문구를 `.form-hint`에 복원.
- 하단 문구가 너무 도드라진다는 피드백에 따라 `.auth-card .form-hint`를 가운데 정렬 + 옅은 회색(`#9AA5B1`)으로 스코프 지정. `.form-hint`는 lab.html 등에서도 쓰는 공용 클래스라 `.auth-card` 범위로 한정해 다른 페이지에는 영향 없음.

### 2. 공유소스 '중요' 체크가 저장되지 않던 버그 수정 (`supabase-schema.sql`, `lab.html`, `lab-resources.html`)

- 공지사항은 '중요' 체크가 정상 동작했지만 공유소스는 체크해도 반영되지 않던 문제.
- 원인 3중첩:
  1. `resources` 테이블에 `is_important` 컬럼 자체가 없었음.
  2. 저장 시 체크박스 값을 payload에 담지 않고 있었음 (`lab.html` `saveLabForm`).
  3. 불러올 때 `normalizeResource()`가 `important: false`로 하드코딩되어 있어 DB 값이 있어도 항상 꺼진 것처럼 보임.
  4. `lab-resources.html`(공유소스 전체보기)은 필드명을 `resource.important`로 잘못 참조해 공지사항 쪽(`is_important`)과 이름이 안 맞았음.
- `resources.is_important` 컬럼 추가(schema + 실 DB에 `ALTER TABLE`로 반영) 후 위 세 지점 모두 수정.

### 3. 임시 비밀번호 로그인 사용자용 비밀번호 변경 안내 팝업 추가 (`login.html`)

- `CAP_이름` 형태 임시 비밀번호로 로그인 성공 시, 곧장 리다이렉트하지 않고 "임시 비밀번호를 변경해 주세요" 모달을 먼저 표시.
- "지금 변경하기" → `settings.html`(개인설정)로 이동, "나중에" → 원래 이동하려던 페이지로 진행.
- 팝업형 / 상단 배너형 두 가지 시안을 제시했고, 팝업형으로 최종 결정.

### 4. 개인 연구페이지 메모/피드백 '확인 완료' 체크가 새로고침 시 초기화되던 버그 수정 (`supabase-schema.sql`, `supabase-client.js`, `lab-member.html`)

- `member_notes` 테이블에 `done` 컬럼이 없어 체크박스가 로컬 상태만 토글하고 저장되지 않던 문제(코드에 "테이블에 done 컬럼이 없으므로 로컬 상태만 토글" 주석이 남아 있었음). 같은 역할의 `team_project_notes.done`은 이미 정상 동작 중이었음.
- `member_notes.done` 컬럼 추가, `CAPData.updateMemberNote()` 신설, `toggleNoteDone()`이 실제로 서버에 저장하고 실패 시 체크 상태를 롤백하도록 수정.
- 메모 수정 시 기존에는 "삭제 후 재삽입" 방식이라 수정할 때마다 `done` 값이 초기화될 뻔한 부분도 `updateMemberNote()`로 교체해 함께 수정.

---

## 2026-07-15 — "1일 전" 알림이 예상보다 일찍 온 문제 원인 조사 (코드 수정 없음)

### 증상

- 내일 10시 미팅에 대한 "1일 전" 알림이 오전 10시가 아니라 오전 9시 5분에 도착.

### 원인

- `supabase/functions/push-reminders/index.ts`(및 클라이언트 폴백 `cap-notifications.js`)의 `TIMING_WINDOWS`에서 `day1`은 "이벤트 시각 정각 24시간 전"이 아니라 **"이벤트 시작 23~25시간 전" 사이의 2시간짜리 상대적 창**(`low:1380, high:1500`)으로 정의되어 있음.
- 내일 10:00 미팅이면 이 창은 오늘 09:00~11:00에 걸쳐 있고, `notification_dispatch_log`의 `(kind, ref_id)` unique constraint로 인해 5분 주기 크론이 **창에 처음 진입하는 순간(09:00~09:05경) 즉시 발송하고 중복 방지 처리**됨 → 항상 정각보다 최대 1시간 일찍 도착할 수 있는 구조.
- 타임존 처리(+09:00 오프셋 명시)와 크론 표현식(`*/5 * * * *`)은 정상, 버그 아님.

### 결정: 코드 수정하지 않음

- `day1`의 2시간 폭은 의도적으로 유지하기로 결정(사용자 확인).

### 다른 타이밍 창 재점검 (문제 없음 확인)

| 알림 | 창 | 폭 | 비고 |
|---|---|---|---|
| `day1` (1일 전) | 23~25h 전 | 120분 | 상대적 창, 유지 결정 |
| `morning9` (당일 오전 9시) | KST 09:00~09:14 | 14분 | `minutesUntil` 아닌 절대 시각(`nowKSTMinutes`) 비교라 별개 로직 |
| `min30` (30분 전) | 27~33분 전 | 6분 | 목표(30분) 중심 ±3분, 5분 크론이 최소 1회 창을 걸치기 위한 최소 폭 |
| `min15` (15분 전) | 12~18분 전 | 6분 | 목표(15분) 중심 ±3분 |
| `atStart` (시작 시간) | 0~5분 전 | 5분 | 시작 직전만 |

- `min30`/`min15`/`atStart`/`morning9`은 이미 정확한 시각 기대에 맞는 좁은 창으로 설계돼 있어 추가 수정 불필요.

---

## 2026-07-09 — 일정/회의록 기능 개선 4건 + 회의록 중복 생성 방지

### 1. 참석자 체크박스 정렬 버그 수정 (`style.css`)

- PC 화면에서 일정 추가/수정 창의 참석자 체크박스가 이름과 나란히 놓이지 않고 체크박스만 위로, 이름은 아래 줄로 밀려 어긋나 보이던 문제 (스크린샷으로 확인).
- 원인: `.form-group input` 규칙이 `width:100%`를 체크박스에도 적용해 체크박스가 셀 전체 너비로 늘어나며 뒤에 오는 이름 텍스트를 다음 줄로 밀어냈음. `.member-check`의 `flex` 정렬도 `.form-group label { display:block }`에 명시도 싸움에서 밀려 무력화되고 있었음.
- `.form-group input`에서 `checkbox`/`radio` 타입 제외, `.member-check`를 `.form-group .member-check`로 스코프해 명시도를 높여 정렬 복구.
- 영향 범위: `lab.html`(일정 참석자 선택), `lab-project.html`(팀 프로젝트 참여 구성원 선택) 모두 동일 구조라 함께 수정됨.

### 2. 회의록 추가 시 선택된 날짜의 랩미팅 자동 연결 (`lab.html`)

- 캘린더에서 특정 날짜를 선택한 상태로 "회의록 추가"를 누르면, 그 날짜에 해당하는 랩미팅이 연결 일정 드롭다운에 자동으로 선택되도록 `populateMeetingSelectIfOpen()`을 수정.

### 3. 일정 상세 팝업에 "회의록 추가" 버튼 신설 (`lab.html`, `style.css`)

- 랩미팅 일정을 클릭했을 때 뜨는 수정 팝업의 "수정" 버튼 옆에 "회의록 추가" 버튼 추가 (랩미팅 타입 일정에서만 노출).
- 클릭하면 해당 일정이 연결 랩미팅으로 미리 선택된 채 회의록 작성 창이 바로 열림.
- 버튼이 3개로 늘며 서로 붙어 보이던 `.event-modal-actions`에 `gap: 8px` 추가.

### 4. 회의록 연결 일정 드롭다운을 최신순으로 정렬 (`lab.html`)

- 기존에는 연결 가능한 랩미팅 목록이 오래된 순으로 나열되어 최근 미팅을 찾으려면 스크롤을 내려야 했음 → 최신 날짜가 먼저 오도록 정렬 순서를 뒤집음.

### 5. 회의록 중복 생성 방지 (추가 요청 반영, `lab.html`)

- 2번·3번 기능으로 회의록 추가를 열 때마다 이미 그 미팅에 연결된 회의록이 있어도 매번 새 회의록이 만들어지는 문제가 있었음 — 하나의 미팅에는 회의록이 하나만 존재해야 함.
- `openLabDrawer('minutes')`가 열릴 때 대상 미팅(버튼으로 지정된 일정 또는 캘린더 선택 날짜의 미팅)에 이미 연결된 회의록이 있으면, 새로 만들지 않고 그 회의록의 수정 화면으로 바로 전환하도록 수정.
- 혹시 드롭다운에서 이미 회의록이 있는 미팅을 수동으로 골라 저장하는 경우에도, 중복 insert 대신 기존 레코드를 update하도록 `saveLabForm`에 안전장치 추가.

### 6. 캐시 무효화

- `lab.html`, `lab-project.html`의 `style.css` 쿼리 버전을 `member-check-align-v1`로 갱신해, 캐시된 구버전 CSS 때문에 체크박스 정렬 수정이 반영 안 되는 일이 없도록 처리.

### 7. 배포 확인

- 커밋 후 GitHub `main`에 push, GitHub Pages 빌드 성공 확인.

---

## 2026-07-02 진행 요약 (3) — LAB 페이지 공지/공유소스 패널에 전체 개수 표시 + 페이지네이션 추가

### 문제

- LAB 페이지의 공지사항·공유 소스 패널은 최신 2건만 보여주도록 개수를 제한해 두었는데, 전체 게시글이 몇 개인지 알려주는 표시가 전혀 없어 화면에 보이지 않는 글이 더 있는지 사용자가 알 방법이 없었음 (예: 공지사항 총 3건 중 2건만 노출, 나머지 1건의 존재 여부를 알 수 없음).

### 해결 (`lab.html`)

- **전체 개수 안내 문구**: 각 패널 제목 아래에 `.lab-panel-sub`(회의록 패널에 이미 쓰이던 서브타이틀 스타일 재사용)로 "전체 N건 중 X-Y번째 표시" 문구를 추가 — 별도 조작 없이도 숨겨진 글이 있는지 바로 인지 가능.
- **패널 내 페이지네이션**: 각 리스트 하단에 회의록/팀 프로젝트 패널과 동일한 `.minutes-pagination` 컴포넌트("이전 / N of M / 총 X개 / 다음")를 재사용해 추가 — `전체보기` 페이지로 이동하지 않고도 LAB 페이지 안에서 나머지 글을 바로 확인 가능.
- `noticePage`/`NOTICE_PAGE_SIZE`, `resourcePage`/`RESOURCE_PAGE_SIZE` 상태 변수 추가, 표시 개수(2개)는 기존과 동일하게 유지.
- 신규 공지/공유 자료를 추가하면 해당 패널이 1페이지로 자동 이동해 방금 등록한 글이 바로 보이도록 처리.

---

## 2026-07-02 진행 요약 (2) — 공지사항 카드 높이 고정, 전체보기 페이지 신설, 작성일 버그 수정

### 1. 공지사항 카드 높이 고정 (`style.css`)

- LAB 페이지 사이드 위젯(`#notice-list`)의 공지 카드가 본문 분량에 따라 카드 높이가 계속 늘어나던 문제 → `-webkit-line-clamp`로 제목 1줄 / 본문 미리보기 2줄로 잘라내어 카드 높이를 고정.
- 공지사항 전체보기 페이지(`lab-notices.html`)의 목록 카드도 동일하게 제목 1줄 / 본문 미리보기 3줄로 클램프 적용해 모든 공지 카드가 동일한 높이를 유지.

### 2. 공지사항 상세 페이지 신설 (`lab-notice-detail.html`)

- 기존에는 전체보기 페이지의 공지 셀을 클릭하면 LAB 페이지와 동일한 모달 팝업이 화면 중앙에 뜨는 방식이었으나, 요청에 따라 **모달 대신 별도 페이지로 이동**하는 방식으로 변경.
- `lab-notice-detail.html?id=<공지ID>` 신설 — 사이트 공통 헤더/푸터 레이아웃 안에서 공지 제목/작성일/전체 본문을 페이지 전체 보기로 표시. "목록으로" / "LAB으로 돌아가기" 링크 포함.
- `lab-notices.html`의 카드를 `<button onclick=...>` → `<a href="lab-notice-detail.html?id=...">`로 교체 (모달 관련 마크업/JS 제거).
- LAB 페이지(`lab.html`) 자체의 공지 클릭 시 모달 팝업 동작은 기존 그대로 유지 — 이번 변경은 전체보기 페이지의 클릭 동작에만 적용.

### 3. 공지 카드 폰트 축소 회귀 수정 (`style.css`)

- 위 1번 작업 중 카드를 `<button>` 엘리먼트로 바꿨더니, 브라우저 기본 폼 컨트롤 폰트 크기(약 13px)를 명시적 `font-size`가 없던 본문 `<p>`가 그대로 상속하면서 전체보기 페이지의 리스트 폰트가 눈에 띄게 작아지는 회귀가 발생.
- 카드를 `<a>` 태그로 교체(2번 작업)하면서 폼 컨트롤 폰트 축소 문제가 근본적으로 해결됐고, 추가로 제목 1.02rem→1.14rem, 날짜 0.8rem→0.86rem, 본문 미리보기 1rem(명시적 지정)로 기존보다 살짝 키움.

### 4. 공지 작성일 '일' 표시 오류 수정 (`lab.html`)

- **버그 1 (NaN 표시)**: 공지 상세 모달(`openDetailModal`)이 전체 ISO 타임스탬프(`item.createdAt`, 예: `2026-07-02T05:23:11+00:00`)를 `formatKoreanDate()`에 그대로 넘기고 있었음. 이 함수는 `'YYYY-MM-DD'` 형태만 기대하고 `split('-')`로 파싱하므로, 타임존 오프셋의 콜론·플러스 기호가 섞이며 일(day) 값이 `NaN`으로 표시됨 → `item.date`(정제된 날짜 문자열)를 넘기도록 수정.
- **버그 2 (하루 오차)**: `normalizeNotice()`가 `created_at`(UTC 타임스탬프)의 날짜 부분을 로컬 타임존 변환 없이 그대로 잘라 썼음(`createdAt.split('T')[0]`) → 한국시간 새벽 0~9시 사이에 작성된 공지는 UTC 기준으로 전날 날짜가 되어 하루 앞선 날짜로 표시되는 문제 발생 → `new Date(createdAt)` + 로컬 타임존 기준 `formatDateKey()`로 계산하도록 수정.

### 5. 캐시 버스팅 / 서비스워커 버전 갱신

- 위 CSS·HTML 변경사항이 반영되도록 `style.css` 쿼리 버전을 `lab.html` / `lab-notices.html` / `lab-notice-detail.html`에서 `notice-clamp-v1`로 통일.
- `sw.js`의 `CACHE_NAME`을 `v20` → `v21`로 상향해 서비스워커가 새로 설치되며 이전 캐시를 정리하도록 함.
...
- 업로드 가능 파일 크기 제한 **1MB → 5MB** 상향
- 안내 문구 및 에러 메시지 일치하게 수정

### 2. 프로필 사진 표시 개선 (`style.css`)
- `.avatar`에 `overflow: hidden` 추가 → 사진이 테두리 바깥으로 삐져나오지 않음
- `.avatar-photo`에 명시적 픽셀 크기 지정 → 사진이 아바타 칸에 꽉 차게 표시

### 3. 아바타 칸 크기 조정 (`style.css`)
| 대상 | 변경 후 |
|---|---|
| 교수 | 124 × 124 px |
| 일반 구성원 | 100 × 100 px |
| 졸업생 | 88 × 88 px (별도 적용) |
| 프로필 편집 미리보기 | 100 × 100 px |
| lab-member 페이지 | 100 × 100 px |

### 4. 조합형 캐릭터 아바타 제거 + 이모지 배경색 기능 추가 (`member-dashboard.html`)
- 프로필 편집 페이지에서 **조합형 캐릭터 아바타** 섹션 전체 제거 (`character-builder`, `getCharacterAvatar` 등)
- **이모지 아바타 배경색 선택** 기능 추가: 6가지 컬러 스와치(하늘색·민트·연노랑·연분홍·라벤더·그레이)
  - `avatarBg` 필드로 저장·로드, 미리보기 즉시 반영
  - hex 정규식 검증으로 XSS 방지
- `people.html`에도 `avatarBg` 적용 → 구성원 페이지 이모지 아바타에 배경색 표시

### 5. lab-member 페이지 사진 표시 버그 수정 (`lab-member.html`)
- 기존 코드가 `profile.photo`를 무시하고 이모지만 표시하던 문제 수정
- `photo`가 있으면 `<img>` 태그로 표시, 없으면 이모지/이름 첫 글자로 폴백

### 6. 연구 관심사 필드 제목 수정 (`member-dashboard.html`)
- 일반·재학 구성원 대상 레이블 `'연구 관심사'` → **`'연구 관심사 및 자기소개'`**
- 졸업생 레이블(`'현재 소속'`)은 유지
...
## 2026-06-19 — LAB 알림 문구, 공지/공유 소스 카드, 회의록 표시 방식 정리

오늘 반영한 내용은 모두 커밋 후 `main`에 push했고, GitHub Pages 배포 성공까지 확인했다. Supabase Edge Function 변경이 포함된 항목은 해당 함수 배포도 완료했다.

### 1. 일정 푸시 알림 body 문구 위치 변경

- `supabase/functions/push-on-event/index.ts`: 일정 생성/수정 푸시 알림의 body 문구를 `새 일정이 추가되었습니다 (랩미팅)` 형식에서 `[랩미팅] 새 일정이 추가되었습니다` 형식으로 변경.
- 괄호 안에 있던 일정 유형을 대괄호로 감싸 문장 맨 앞으로 이동했다.
- Supabase Edge Function `push-on-event` 재배포 완료. (`7be21f2`)

### 2. 공지사항 작성일 자동 처리

- `lab.html`: 공지 추가/수정 폼에서 사용자가 직접 입력하던 `공지 날짜` 필드를 제거.
- 공지 날짜 표시는 Supabase `notices.created_at` 기준으로 통일했다.
- 공지 상세 모달의 날짜 라벨은 `작성일`로 정리했다. (`7be21f2`)

### 3. LAB 메인 공지사항 카드 표시 구조 정리

- `lab.html`, `style.css`: LAB 메인 공지사항 카드가 `공지사항 전체보기` 페이지와 같은 흐름으로 보이도록 `날짜/중요 표시 → 제목 → 내용` 순서로 정리.
- 제목 폰트와 카드 내부 간격을 전체보기 카드 기준에 맞춰 조정했다. (`eada834`, `2130767`)

### 4. LAB 메인 공유 소스 카드 표시 구조 정리

- `lab.html`, `style.css`: LAB 메인 공유 소스 카드를 `공유 소스 전체보기` 카드와 같은 흐름으로 `태그 → 제목` 중심 구조로 정리.
- LAB 메인 카드에서는 설명 문단이 보이지 않도록 확정했다.
- `lab-resources.html` 전체보기 페이지에서는 설명 문단이 계속 보이도록 복구해 최종 상태를 확정했다. (`9997638`, `2130767`, `322323e`, `9add797`)

### 5. 회의록 본문 줄바꿈/넘침 방지

- `style.css`: `.minutes-card p`에 `white-space: pre-wrap`, `overflow-wrap: anywhere`, `word-break: break-word` 적용.
- 회의록 내용이 길어도 정해진 카드 폭을 벗어나지 않고 자동 줄바꿈되도록 수정.
- 작성자가 입력한 줄바꿈도 보존된다. (`9997638`)

### 6. 배포 확인

- 각 수정 커밋은 모두 GitHub `main`에 push 완료.
- GitHub Pages `pages build and deployment`가 `completed / success`로 끝난 것을 확인.
- 최종 관련 커밋:
  - `7be21f2` — 알림 문구 및 공지 작성일 자동 처리
  - `eada834` — 공지 카드 제목 레이아웃 1차 조정
  - `9997638` — 공유 소스 제목/회의록 줄바꿈 조정
  - `2130767` — 공지/공유 소스 카드를 전체보기 페이지 구조에 맞춤
  - `322323e` — LAB 메인 공유 소스 카드 설명 숨김
  - `9add797` — 공유 소스 전체보기 페이지 설명 표시 복구

---

## 2026-06-24 — 구성원 로그인 계정 생성/삭제 정책 확정

관리자 패널에서 구성원을 추가·수정·삭제할 때 Supabase `members` 행과 Supabase Auth 계정이 서로 어긋나던 문제를 정리했다. 오늘 확정한 방향은 **학번이 있는 구성원은 학번으로 로그인**, **학번 미지정 구성원은 이름 + `CAP_이름` 임시 비밀번호로 로그인**이다.

### 1. 학번 등록 구성원 로그인 복구

- 운영 DB에서 `student_id`는 있지만 `auth_user_id`가 비어 있던 구성원을 확인했다.
- `김민지(2025192701)`, `이수빈(2025192715)`의 Supabase Auth 계정을 생성하고 `members.auth_user_id`와 연결했다.
- 두 사람 모두 `학번@kwcaplab.internal / 비밀번호=학번` 방식으로 실제 로그인 성공까지 확인했다.

### 2. 학번 미지정 구성원 임시 로그인 정책

- 학번 없이 구성원을 추가하면 내부 식별용 `student_id`는 `TBD_이름`으로 생성된다.
- 사용자가 실제로 입력해야 하는 임시 비밀번호는 `CAP_이름`으로 확정했다.
- `TBD_이름`은 Supabase Auth 이메일을 만들고 계정을 찾기 위한 내부 ID일 뿐이므로, 관리자 화면에서는 학번처럼 노출하지 않도록 숨겼다.
- 관리자 패널 구성원 목록에서는 학번 미지정 구성원을 빈 학번 칸 + placeholder `학번 미지정 · 임시 비밀번호 CAP_이름` 형식으로 표시한다.
- 기존 `TBD_...` 구성원들의 Auth 비밀번호도 `CAP_이름`으로 일괄 재설정했다.
- 예시 검증: `박정은 / CAP_박정은` 로그인 성공, 기존 `TBD_박정은` 비밀번호 로그인은 차단됨.

### 3. 동명이인 생성 허용

- 같은 이름의 학번 미지정 구성원을 다시 추가하면 기존에는 `student_id = TBD_이름` unique 제약과 Auth 이메일 중복 때문에 생성이 실패했다.
- `create-member` Edge Function에서 같은 이름의 임시 구성원이 이미 있으면 내부 ID를 `TBD_이름_2`, `TBD_이름_3`처럼 자동 증가시키도록 수정했다.
- 이에 맞춰 임시 비밀번호도 `CAP_이름`, `CAP_이름_2`, `CAP_이름_3`처럼 표시·검증되도록 정리했다.
- 운영 함수로 같은 이름의 테스트 구성원 2명을 생성해 각각 로그인 성공을 확인했고, 테스트 데이터는 즉시 삭제했다.

### 4. 구성원 삭제 시 Auth 계정 동시 삭제

- 관리자 패널에서 구성원을 삭제할 때 `members` 행만 삭제되던 흐름을 수정했다.
- `create-member` Edge Function에 `delete-member` 액션을 추가해 `members.auth_user_id`를 읽은 뒤 DB 행과 Supabase Auth 유저를 함께 삭제하도록 했다.
- 운영 함수로 테스트 구성원을 생성한 뒤 삭제 검증:
  - 삭제 전 로그인 성공
  - 삭제 후 `members` DB 행 0건
  - 삭제 후 Auth 로그인 차단 확인

### 5. 화면 문구 정리

- 로그인 화면에서 임시 비밀번호 안내 문구는 삭제했다.
- 단, 기존 안내였던 `교수님은 관리자 계정 비번을 사용하세요.` 문구는 다시 표시하도록 복구했다.
- 새 구성원 추가 폼의 학번 placeholder는 `비우면 임시 비밀번호 CAP_이름 생성`으로 정리했다.
- `TBD_이름`은 내부 식별자라 사용자가 볼 필요가 없으므로 문구에서 제거했다.

### 6. 배포 및 검증

- Supabase Edge Function `create-member` 재배포 완료.
- GitHub `main`에 관련 커밋 push 완료.
- 실제 도메인 `kwcaplab.co.kr`에서 캐시 우회 URL로 `login.html`, `admin.html`, `supabase-client.js?v=supabase-v5` 반영 확인.
- 최종 관련 커밋:
  - `b1b4882` — 학번 등록 구성원 Auth 연결 및 관리자 학번 변경 동기화
  - `1937055` — 구성원 삭제 시 Auth 계정 동시 삭제, 로그인 임시 안내 문구 삭제, 관리자 화면 문구 정리
  - `0c2ca77` — 동명이인 임시 구성원 생성 허용, 교수님 로그인 안내 문구 복구
